...
class BidirectionalGRU(EncoderBase):
    def __init__(self, xdim, hdim, zdim, encode_d=True):
        ...
        self.u_net.add(tf.keras.layers.Bidirectional(tf.keras.layers.GRU(hdim, return_sequences=True)))
        ...
        if encode_d:    # might not need this for inference models to test ELBO vs Kalman loss functions 
            ...
            self.d_net.add(tf.keras.layers.Bidirectional(tf.keras.layers.GRU(hdim, return_sequences=True)))
